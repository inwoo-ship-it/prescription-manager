...
async function applyStatus(id, memberIds) {
  ...
  // ID가 비어있으면 캐시 문제 — 새로고침 유도
  if (!id || id === 'undefined' || id === '') {
    toast('⚠️ ID 없음 — 새로고침 후 다시 시도하세요', 'err');
    sessionStorage.removeItem(CACHE_KEY);
    return;
  }
...
async function changeStatus(id, newStatus, memberIds, selectEl) {
  ...
  if (!g) { toast('항목을 찾을 수 없습니다 — 새로고침 해주세요', 'err'); return; }
  const old = g.status;

  // 낙관적 업데이트
  g.status = newStatus;
  g.member_ids?.forEach(mid => { const r = allData.find(x=>x.id===mid); if(r) r.status = newStatus; });
  if (selectEl) selectEl.className = `status-sel s-${newStatus}`;
  updateStats();

  const isGroup = memberIds.length > 1;
  const params  = isGroup
    ? { action:'updateGroupStatus', ids: JSON.stringify(memberIds), status:newStatus }
    : { action:'updateStatus', id: String(id), status:newStatus };
  ...
  try {
    ...
    try { res = JSON.parse(text); } catch { res = { success: true }; }
  } catch(e) { res = { error: e.message }; }

  if (res.error && res.error !== '행 없음') {
    g.status = old;
    if (selectEl) { selectEl.value = old; selectEl.className = `status-sel s-${old}`; }
    updateStats();
    toast('❌ 변경 실패: ' + res.error, 'err');
  } else if (res.error === '행 없음') {
    toast('⚠️ 행 없음 — ID가 시트와 불일치. 새로고침 후 재시도', 'err');
    sessionStorage.removeItem(CACHE_KEY);
    g.status = old;
    if (selectEl) { selectEl.value = old; selectEl.className = `status-sel s-${old}`; }
    updateStats();
  } else {
    toast(`✅ ${g.pharmacy_name} → ${newStatus}`, 'ok');
    if (old !== newStatus) render();
  }
}
